...
val inStock = for(head :: tail <- baskets if head == "Bread") yield "Sliced Bread" :: tail

val inStock = baskets withFilter {
  case head :: tail => head == "Bread"
} map { 
  case head :: tail => "Sliced Bread" :: tail 
}


val inStock = baskets withFilter { e => e match
  case head :: tail => head == "Bread"
  case _ => false
} map { e => e match 
  case head :: tail => "Sliced Bread" :: tail 
