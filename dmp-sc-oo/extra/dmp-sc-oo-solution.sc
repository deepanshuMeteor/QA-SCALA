...
//OR,
println(basket.foldLeft(0.0) { _ + _.price })
//println(basket reduce { _ + _.price })
...
class Detective(val fullname: String)
// class Detective private (val fullname: String, val address: String)

object Detective {
  def apply(info: String) = new Detective(info.split(",")(0))

  def unapply(d: Detective): Option[(String, String)] =  Some(
...
//apply -- RHS
val d = Detective.apply("Sherlock Holmes, 22B Baker St.")

//unapply -- LHS
val Some(firstname, secondname) = Detective.unapply(d)


println(d match {
  case Detective(a, b) => a + b 
  case _ => "?"
})
