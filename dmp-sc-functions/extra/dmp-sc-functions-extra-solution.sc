...
def formatMessages(ms: List[String], f: String => String): List[String] =
  for(m <- ms) yield "MSG: " + f(m)
  

//Q. define a function for use with formatMessages 
// assign it to fmt
// it should uppercase its input
// HINT: this function should have one argument

val fmt  = (s: String) => s.toUpperCase

//Q. what is the type of fmt?
//... add it to the val fmt
//val fmt : String => String
//ASIDE: val fmt : String => String = _.toUpperCase

//Q. define a val results which is a formatted list of messages
// given by msgs
//HINT: use formatMessages with fmt
val msgs = List("Hi!", "bye!", "Sigh?")


val results = formatMessages(msgs, fmt)
...
/*
{
  val message = formatter(level, message)
  println(message)
}
*/


// now call printError with join, where the level 
// is error and the message is "bad"

printError("error", "bad", join)
